  ...
  for (const [alias, targets] of Object.entries(paths)) {
    ...
    if (isWildcard && importPath.startsWith(aliasPrefix + "/")) {
      ...
      for (const target of targets) {
        ...
        const found = tryResolveFile(
          path.resolve(pathsBaseDir, targetBase, suffix),
        );
...
export function resolveImportToFiles(
  ...
  if (specifier.includes("*")) {
    return resolveGlob(specifier, sourceFile, tsAliases, allFiles).filter((f) =>
      allFilesSet.has(f),
    );
  }
  ...
  if (specifier.startsWith(".")) {
    ...
    Object.keys(tsAliases?.paths ?? {}).length > 0 &&
    (resolved = resolveAliasedImport(specifier, tsAliases))
  ) {
