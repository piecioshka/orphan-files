...
export function analyze(allFiles, fileImports, options = {}) {
  ...
  const entries = new Set(
    collectPackageEntryFiles(projectDir, allFiles, allFilesSet),
  );
  for (const file of allFiles) {
...
export function findUnusedFiles(
  ...
  return analyze(allFiles, fileImports, {
    projectDir,
    entryPatterns: exceptionPatterns,
  }).unused;
}
...
export function explainFile(targetFile, result, projectDir = process.cwd()) {
  ...
  if (entries.has(targetFile)) {
    return {
      file: targetFile,
      status: "entry",
      reason: "declared/detected as an entry point",
      path: [targetFile],
    };
  }
