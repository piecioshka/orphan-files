...
import { DEFAULT_CONFIG, loadUserConfig } from './config.js';
import {
    buildUnusedEntries,
    formatCli,
    formatJson,
...
function initConfig(cwd) {
    const target = path.join(cwd, 'orphan-files.config.js');
    if (fs.existsSync(target)) {
        console.error('Config already exists: orphan-files.config.js');
        return 1;
    }
    fs.writeFileSync(target, CONFIG_TEMPLATE);
    console.log(`Created orphan-files.config.js`);
    return 0;
}
...
async function gatherFiles(cwd, config, respectGitignore) {
    ...
    for (const pattern of config.include) {
        const files = await scanProject(cwd, pattern, config.exclude, { respectGitignore });
        all.push(...files);
    }
    return [...new Set(all)];
}
...
function generatePdf(markdown, cwd, hasUnused) {
    ...
    try {
        ...
        console.error('PDF generation failed. Make sure md-to-pdf is available (npx md-to-pdf).');
...
export async function run(rawArgv = process.argv.slice(2)) {
    const argv = minimist(rawArgv, {
        boolean: ['help', 'version', 'init', 'fix', 'force', 'dry-run', 'group', 'gitignore'],
        alias: { c: 'config', f: 'format', h: 'help', v: 'version' },
        default: {
            config: 'orphan-files.config.js',
            format: 'cli',
            sort: 'path',
            gitignore: true,
        },
    ...
    const quiet = argv.format !== 'cli' || Boolean(argv.why) || Boolean(argv.graph);
    ...
    try {
        ...
        if (userConfig) {
            ...
            if (!quiet) console.log(`Loaded config from ${argv.config}`);
        }
    } catch (error) {
    ...
    const analysis = analyze(files, fileImports, { projectDir: cwd, entryPatterns });
    ...
    if (argv['update-baseline']) {
        const baselinePath = path.resolve(cwd, typeof argv['update-baseline'] === 'string'
...
function runFix(entries, cwd, force) {
    ...
    if (!force) {
        console.log(`Would delete ${entries.length} files (re-run with --force to delete):`);
