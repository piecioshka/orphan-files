...
export function detectFrameworkEntries(projectDir) {
    ...
    if (allDeps['next']) {
        entries.push(
            'next.config.*', 'next-env.d.ts', 'middleware.{ts,js}',
            'instrumentation.ts', 'instrumentation-client.ts',
        );
        const appSpecialFiles = [
            'sitemap.{ts,js}', 'manifest.{ts,js}', 'robots.{ts,js}',
            'page.{ts,tsx,js,jsx}', 'layout.{ts,tsx,js,jsx}',
            'route.{ts,tsx,js}', 'loading.{ts,tsx,js,jsx}',
            'error.{ts,tsx,js,jsx}', 'global-error.{ts,tsx,js,jsx}',
            'not-found.{ts,tsx,js,jsx}', 'template.{ts,tsx,js,jsx}',
            'default.{ts,tsx,js,jsx}', 'opengraph-image.{ts,tsx}',
            'twitter-image.{ts,tsx}', 'icon.{ts,tsx}',
        ];
    ...
    if (Object.keys(allDeps).some(k => k === 'storybook' || k.startsWith('@storybook/'))) {
    ...
    if (allDeps['vite'] || allDeps['@sveltejs/kit'] || allDeps['astro'] || allDeps['nuxt']) {
        entries.push('vite.config.*', 'src/main.{ts,tsx,js,jsx}', 'src/routes/**', 'src/pages/**');
    }
...
function packageEntriesFor(pkgDir, allFiles, allFilesSet) {
    ...
    for (const c of candidates) resolveCandidate(c, pkgDir, allFiles, allFilesSet, found);
    return [...found];
}
...
export function collectPackageEntryFiles(projectDir, allFiles, allFilesSet) {
    ...
    for (const dir of dirs) {
        for (const f of packageEntriesFor(dir, allFiles, allFilesSet)) entries.add(f);
    }
    return entries;
}
