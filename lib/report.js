    ...
    return JSON.stringify({
        ...
        runs: [{
            ...
            results: entries.map(e => ({
                ruleId: 'unused-file',
                level: 'warning',
                message: { text: `'${e.rel}' is not reachable from any entry point.` },
                locations: [{
                    physicalLocation: {
...
export function formatPdfMarkdown(entries, { projectDir, totalFiles, dateString }) {
    const lines = [
        '# orphan-files report',
        '',
        `**Date:** ${dateString}`,
    ...
    if (entries.length === 0) {
        ...
        const totalSize = entries.reduce((sum, e) => sum + e.size, 0);
        lines.push(`## Unused files (${entries.length}) — ${formatBytes(totalSize)} reclaimable`, '');
        entries.forEach(e => lines.push(`- \`${e.rel}\` (${formatBytes(e.size)})`));
    }
    return lines.join('\n');
}
...
export function formatGraph(analysis, { projectDir, format = 'mermaid' }) {
    ...
    if (format === 'dot') {
        ...
        for (const [file, id] of ids) {
            const color = unused.has(file) ? ' color="red"' : analysis.entries.has(file) ? ' color="green"' : '';
            lines.push(`  ${id} [label="${label(file)}"${color}];`);
        }
        for (const [file, targets] of analysis.graph) {
            for (const target of targets) lines.push(`  ${ids.get(file)} -> ${ids.get(target)};`);
        }
        lines.push('}');
        return lines.join('\n');
    }
    ...
    for (const [file, targets] of analysis.graph) {
        for (const target of targets) mermaidLines.push(`  ${ids.get(file)} --> ${ids.get(target)}`);
    }
    for (const [file, id] of ids) {
        if (unused.has(file)) mermaidLines.push(`  class ${id} unused;`);
        else if (analysis.entries.has(file)) mermaidLines.push(`  class ${id} entry;`);
    }
