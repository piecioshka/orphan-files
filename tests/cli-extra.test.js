...
describe('cli — new features', () => {
    ...
    describe('--init', () => {
        it('creates a config file and exits 0', async () => {
            ...
            expect(fs.existsSync(path.join(tmpDir, 'orphan-files.config.js'))).toBe(true);
    ...
    describe('--fix', () => {
        ...
        it('reports an error when a file cannot be deleted', async () => {
            write('a.js', '// nothing');
            const rmSpy = vi.spyOn(fs, 'rmSync').mockImplementation(() => { throw new Error('EPERM'); });
            await cli(tmpDir, '--fix', '--force', '--config', 'nope.js');
            expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Failed to delete'));
            rmSpy.mockRestore();
        });
    });
    ...
    describe('--baseline / --update-baseline', () => {
        it('writes a baseline and exits 0', async () => {
            ...
            expect(JSON.parse(fs.readFileSync(baselinePath, 'utf-8')).unusedFiles).toContain('a.js');
            expect(process.exit).toHaveBeenCalledWith(0);
        });

        it('writes a baseline to an explicit path', async () => {
            write('a.js', '// nothing');
            await cli(tmpDir, '--update-baseline', 'mybase.json', '--config', 'nope.js');
        ...
        it('ignores files recorded in the baseline', async () => {
            write('a.js', '// nothing');
            write('c.js', '// nothing');
            fs.writeFileSync(path.join(tmpDir, 'bp.json'), JSON.stringify({ unusedFiles: ['a.js'] }));
            await cli(tmpDir, '--baseline', 'bp.json', '--format', 'json', '--config', 'nope.js');
            const jsonStr = console.log.mock.calls.map(c => c[0]).find(s => s?.startsWith('{'));
        ...
        it('treats every unused file as fresh when the baseline file is missing', async () => {
            write('a.js', '// nothing');
            await cli(tmpDir, '--baseline', 'missing.json', '--format', 'json', '--config', 'nope.js');
            const jsonStr = console.log.mock.calls.map(c => c[0]).find(s => s?.startsWith('{'));
    ...
    it('honours custom entry patterns from the config file', async () => {
        ...
        write('orphan-files.config.js', `export default { entry: ['custom-entry.js'] };`);
    ...
    it('reports gitignored files when --no-gitignore is set', async () => {
        ...
        await cli(tmpDir, '--no-gitignore', '--format', 'json', '--config', 'nope.js');
        const jsonStr = console.log.mock.calls.map(c => c[0]).find(s => s?.startsWith('{'));
