...
describe('findUnusedFiles', () => {
    describe('with fixture project', () => {
        ...
        it('does not report index.js as unused when it is in exceptions', () => {
            const unused = findUnusedFiles(allFiles, fileImports, ['index.js'], FIXTURE_DIR);
            const unusedNames = unused.map(f => path.basename(f));
        ...
        it('excludes files matching exception patterns', () => {
            const unused = findUnusedFiles(allFiles, fileImports, ['**/unused.js'], FIXTURE_DIR);
            const unusedNames = unused.map(f => path.basename(f));
            expect(unusedNames).not.toContain('unused.js');
        });

        it('excludes spec files via exception pattern', () => {
            const unused = findUnusedFiles(allFiles, fileImports, ['**/*.spec.js'], FIXTURE_DIR);
            const unusedNames = unused.map(f => path.basename(f));
            expect(unusedNames).not.toContain('test.spec.js');
        });

        it('excludes config files via exception pattern', () => {
            const unused = findUnusedFiles(allFiles, fileImports, ['*.config.js'], FIXTURE_DIR);
            const unusedNames = unused.map(f => path.basename(f));
    ...
    describe('with TypeScript path aliases', () => {
        ...
        beforeAll(() => {
            ...
            fs.writeFileSync(path.join(tmpDir, 'src', 'index.ts'), `import { x } from '@/utils';`);
        ...
        it('resolves exact (non-wildcard) alias', () => {
            const tmpDir2 = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-alias-exact-'));
            try {
                fs.mkdirSync(path.join(tmpDir2, 'src'));
                fs.writeFileSync(path.join(tmpDir2, 'src', 'utils.ts'), 'export const x = 1;');
                fs.writeFileSync(path.join(tmpDir2, 'src', 'index.ts'), `import { x } from '@utils';`);
                fs.writeFileSync(path.join(tmpDir2, 'tsconfig.json'), JSON.stringify({
                    compilerOptions: {
                ...
                const unused = findUnusedFiles([indexFile, utilsFile], {
                    [indexFile]: ['@utils'],
                    [utilsFile]: [],
                }, [], tmpDir2);
        ...
        it('resolves wildcard alias where target has no /* suffix', () => {
            const tmpDir2 = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-alias-nonwild-target-'));
            try {
                fs.mkdirSync(path.join(tmpDir2, 'src'));
                fs.writeFileSync(path.join(tmpDir2, 'src', 'utils.ts'), 'export const x = 1;');
                fs.writeFileSync(path.join(tmpDir2, 'src', 'index.ts'), `import { x } from '@/utils';`);
        ...
        it('tries next target when first exact alias target does not exist', () => {
            const tmpDir2 = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-alias-fallback-'));
            try {
                fs.mkdirSync(path.join(tmpDir2, 'src'));
                fs.writeFileSync(path.join(tmpDir2, 'src', 'utils.ts'), 'export const x = 1;');
                fs.writeFileSync(path.join(tmpDir2, 'src', 'index.ts'), `import { x } from '@utils';`);
    ...
    describe('tsconfig edge cases', () => {
        it('handles tsconfig with comments', () => {
            ...
            try {
                ...
                fs.writeFileSync(path.join(tmpDir, 'src', 'b.ts'), `import { x } from '@/a';`);
                ...
                const unused = findUnusedFiles([a, b], { [a]: [], [b]: ['@/a'] }, ['b.ts'], tmpDir);
        ...
        it('returns empty aliases when tsconfig has invalid JSON', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-tsconfig-bad-'));
            try {
                fs.writeFileSync(path.join(tmpDir, 'tsconfig.json'), '{ completely: invalid json @@@ }');
        ...
        it('returns empty aliases when tsconfig does not exist', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-no-tsconfig-'));
            try {
                fs.writeFileSync(path.join(tmpDir, 'a.js'), '// nothing');
                const a = path.join(tmpDir, 'a.js');
                const unused = findUnusedFiles([a], { [a]: [] }, [], tmpDir);
        ...
        it('uses empty paths and default baseUrl when tsconfig has no compilerOptions.paths', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-tsconfig-nopaths-'));
            try {
                fs.writeFileSync(path.join(tmpDir, 'tsconfig.json'), JSON.stringify({
                    compilerOptions: {},
                }));
                ...
                const a = path.join(tmpDir, 'a.js');
                const b = path.join(tmpDir, 'b.js');
                const unused = findUnusedFiles([a, b], { [a]: ['./b.js'], [b]: [] }, ['a.js'], tmpDir);
                expect(unused).toHaveLength(0);
            } finally {
                fs.rmSync(tmpDir, { recursive: true });
            }
    ...
    describe('import resolution edge cases', () => {
        ...
        it('returns null from resolveAliasedImport when no alias matches', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-alias-miss-'));
            try {
                fs.mkdirSync(path.join(tmpDir, 'src'));
                fs.writeFileSync(path.join(tmpDir, 'src', 'a.ts'), 'export const x = 1;');
                fs.writeFileSync(path.join(tmpDir, 'src', 'b.ts'), `import { x } from '@components/a';`);
                fs.writeFileSync(path.join(tmpDir, 'tsconfig.json'), JSON.stringify({
                    compilerOptions: { paths: { '@utils/*': ['./src/*'] }, baseUrl: '.' },
                }));
                const a = path.join(tmpDir, 'src', 'a.ts');
                const b = path.join(tmpDir, 'src', 'b.ts');
                // @components/a doesn't match @utils/* alias → resolveAliasedImport returns null → a.ts is unused
                const unused = findUnusedFiles([a, b], { [a]: [], [b]: ['@components/a'] }, [], tmpDir);
        ...
        it('returns null from tryResolveFile when resolved path does not exist', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-noresolve-'));
            try {
                fs.mkdirSync(path.join(tmpDir, 'src'));
                fs.writeFileSync(path.join(tmpDir, 'src', 'a.ts'), 'export const x = 1;');
                fs.writeFileSync(path.join(tmpDir, 'src', 'b.ts'), `import { x } from '@/nonexistent';`);
                fs.writeFileSync(path.join(tmpDir, 'tsconfig.json'), JSON.stringify({
                    compilerOptions: { paths: { '@/*': ['./src/*'] }, baseUrl: '.' },
                }));
                const a = path.join(tmpDir, 'src', 'a.ts');
                ...
                const unused = findUnusedFiles([a, b], { [a]: [], [b]: ['@/nonexistent'] }, [], tmpDir);
                expect(unused.map(f => path.basename(f))).toContain('a.ts');
            } finally {
                fs.rmSync(tmpDir, { recursive: true });
            }
        ...
        it('resolves import with extension added', () => {
            ...
            try {
                ...
                const a = path.join(tmpDir, 'a.js');
                const b = path.join(tmpDir, 'b.js');
                const unused = findUnusedFiles([a, b], { [a]: ['./b'], [b]: [] }, ['a.js'], tmpDir);
                expect(unused).toHaveLength(0);
            } finally {
                fs.rmSync(tmpDir, { recursive: true });
            }
        ...
        it('resolves import via index file in directory', () => {
            ...
            try {
                ...
                fs.writeFileSync(path.join(tmpDir, 'utils', 'index.js'), `export const x = 1;`);
                fs.writeFileSync(path.join(tmpDir, 'a.js'), `import './utils';`);
                const a = path.join(tmpDir, 'a.js');
                const idx = path.join(tmpDir, 'utils', 'index.js');
                const unused = findUnusedFiles([a, idx], { [a]: ['./utils'], [idx]: [] }, ['a.js'], tmpDir);
                expect(unused).toHaveLength(0);
            } finally {
                fs.rmSync(tmpDir, { recursive: true });
            }
    ...
    describe('framework auto-detection', () => {
        it('does not throw when package.json is missing', () => {
            ...
            try {
                fs.writeFileSync(path.join(tmpDir, 'a.js'), '// nothing');
                const a = path.join(tmpDir, 'a.js');
                expect(() => findUnusedFiles([a], { [a]: [] }, [], tmpDir)).not.toThrow();
            } finally {
                fs.rmSync(tmpDir, { recursive: true });
            }
        });
        ...
        it('handles invalid package.json gracefully', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-fw-badpkg-'));
            try {
        ...
        it('adds Next.js exceptions for src/app/ layout', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-fw-next-src-'));
            try {
        ...
        it('adds Remotion exceptions for remotion package', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-fw-remotion-'));
            try {
                fs.mkdirSync(path.join(tmpDir, 'src'));
        ...
        it('adds Remotion exceptions for @remotion/core package', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-fw-remotion2-'));
            try {
                fs.mkdirSync(path.join(tmpDir, 'src'));
        ...
        it('adds exception for entry points found in scripts', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-fw-scripts-'));
            try {
        ...
        it('does not add exceptions for scripts without file paths', () => {
            const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orphan-fw-scripts-nomatch-'));
            try {
