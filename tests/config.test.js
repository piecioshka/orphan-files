...
describe('detectFrameworkEntries', () => {
    ...
    it('includes the stories pattern for @storybook/* packages', () => {
        ...
        try {
            ...
            expect(detectFrameworkEntries(tmpDir)).toContain('**/*.stories.{js,ts,tsx,jsx,mjs}');
        } finally {
            fs.rmSync(tmpDir, { recursive: true });
        }
    });
...
describe('findWorkspacePackageDirs', () => {
    it('returns the dirs declared by workspaces as an array', () => {
        ...
        try {
            ...
            fs.writeFileSync(path.join(tmpDir, 'packages', 'a', 'package.json'), JSON.stringify({ name: 'a' }));
            fs.writeFileSync(path.join(tmpDir, 'packages', 'b', 'package.json'), JSON.stringify({ name: 'b' }));
            fs.writeFileSync(path.join(tmpDir, 'package.json'), JSON.stringify({
                workspaces: ['packages/*'],
            }));
            const dirs = findWorkspacePackageDirs(tmpDir).map(d => path.basename(d)).sort();
    ...
    it('returns the dirs declared by workspaces as an object ({ packages: [...] })', () => {
        ...
        try {
            ...
            const dirs = findWorkspacePackageDirs(tmpDir).map(d => path.basename(d));
    ...
    it('filters out workspace glob matches that are files, not directories', () => {
        ...
        try {
            ...
            fs.writeFileSync(path.join(tmpDir, 'packages', 'realdir', 'package.json'), JSON.stringify({ name: 'realdir' }));
            fs.writeFileSync(path.join(tmpDir, 'packages', 'README.md'), '# not a package');
            fs.writeFileSync(path.join(tmpDir, 'package.json'), JSON.stringify({
                workspaces: ['packages/*'],
            }));
            const dirs = findWorkspacePackageDirs(tmpDir).map(d => path.basename(d));
...
describe('collectPackageEntryFiles', () => {
    ...
    it('ignores non-string export values and non-string scripts', () => {
        ...
        try {
            ...
            const allFiles = [real];
            const result = collectPackageEntryFiles(tmpDir, allFiles, new Set(allFiles));
...
describe('loadUserConfig', () => {
    ...
    it('parses a .json config file into an object', async () => {
        ...
        try {
            ...
            fs.writeFileSync(p, JSON.stringify({ include: ['**/*.js'], exclude: ['**/dist/**'] }));
    ...
    it('loads a .js ESM config via its default export', async () => {
        ...
        try {
            ...
            fs.writeFileSync(p, `export default { include: ['src/**/*.ts'], exceptions: ['index.ts'] };`);
            const cfg = await loadUserConfig(p);
            expect(cfg).toEqual({ include: ['src/**/*.ts'], exceptions: ['index.ts'] });
        } finally {
            fs.rmSync(tmpDir, { recursive: true });
        }
    });
    ...
    it('throws on an unsupported config extension', async () => {
        ...
        try {
            ...
            await expect(loadUserConfig(p)).rejects.toThrow(/Unsupported config extension/);
        } finally {
            fs.rmSync(tmpDir, { recursive: true });
        }
    });
    ...
    it('throws when a .js config does not export a configuration object', async () => {
        ...
        try {
            ...
            await expect(loadUserConfig(p)).rejects.toThrow(/did not export a configuration object/);
        } finally {
            fs.rmSync(tmpDir, { recursive: true });
        }
    });
