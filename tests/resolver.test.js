        ...
        `// root config
{
  ...
  it("resolves a bare `extends` specifier by appending .json (no node_modules file)", () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(nmDir, "shared-config.json"),
        JSON.stringify({
    ...
    try {
      ...
      for (let i = 0; i < chainLen; i++) {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({
            ...next,
            compilerOptions: { paths: { [`@c${i}/*`]: [`./c${i}/*`] } },
          }),
        );
      }
      fs.writeFileSync(
        path.join(dir, "tsconfig.json"),
        JSON.stringify({ extends: "./c0.json" }),
      );
...
describe("resolveImportToFiles", () => {
  ...
  it("resolves a wildcard alias `@/*`", () => {
    ...
    try {
      ...
      const set = new Set(allFiles);
      expect(
        resolveImportToFiles("@/utils", index, aliases, set, allFiles),
      ).toEqual([utils]);
    } finally {
      fs.rmSync(dir, { recursive: true });
    }
  ...
  it("resolves a bare specifier via explicit baseUrl", () => {
    ...
    try {
      ...
      const set = new Set(allFiles);
      expect(
        resolveImportToFiles("services/foo", entry, aliases, set, allFiles),
      ).toEqual([foo]);
    } finally {
      fs.rmSync(dir, { recursive: true });
    }
  ...
  it("resolves a relative glob specifier to multiple matched files in the set", () => {
    ...
    try {
      ...
      const set = new Set(allFiles);
      const result = resolveImportToFiles(
        "./*.js",
        entry,
        {},
        set,
  ...
  it("returns [] for a non-relative glob with no baseUrl", () => {
    ...
    try {
      ...
      expect(
        resolveImportToFiles("pages/*.js", entry, {}, set, allFiles),
      ).toEqual([]);
    } finally {
      fs.rmSync(dir, { recursive: true });
    }
